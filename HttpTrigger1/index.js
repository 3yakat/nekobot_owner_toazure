    ...
    if (req.body && req.body.events[0]) {
      if (
        (req.body.events[0].type === "postback" &&
          req.body.events[0].postback.data === "toys") ||
        req.body.events[0].postback.data === "treat" ||
        req.body.events[0].postback.data === "food"
      ) {
        uploadpath[req.body.events[0].source.userId] =
          req.body.events[0].postback.data;
        return client.replyMessage(req.body.events[0].replyToken, {
          type: "text",
          text:
            req.body.events[0].postback.data + "ですね！動画をUPしてください",
        });
      }
    } else if (
      req.body.events[0].type === "message" &&
      req.body.events[0].message.type === "video"
    ) {
      const ownerid = await getOwnerUrl(req.body.events[0].source.userId);
      const downloadPath =
        "./" + uploadpath[req.body.events[0].source.userId] + ".mp4";

      let getContent = await downloadContent(
